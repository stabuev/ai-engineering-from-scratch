...
Жесткий «leaky step» по-прежнему вычисляет XOR на прямом проходе — для сети с подобранными вручную весами порога достаточно. Но для *обучения* он бесполезен: его производная равна 0 почти всюду (в 0 не определена), и backprop не получает градиента. Сигмоида предпочтительнее, потому что гладкая и всюду имеет ненулевую производную, так что градиентный спуск может двигать веса. Жесткие пороги распространяются вперед, но не обучаются.
