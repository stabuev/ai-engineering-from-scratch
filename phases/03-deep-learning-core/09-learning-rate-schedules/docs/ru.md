...
Каждые `T` шагов расписание прыгает обратно к `lr_max` и снова затухает (проверено: lr при t = 0, T, 2T равен `lr_max`). Периодические рестарты выбивают оптимизатор из острых минимумов; длинная финальная косинусоида даёт ему осесть в плоском.
