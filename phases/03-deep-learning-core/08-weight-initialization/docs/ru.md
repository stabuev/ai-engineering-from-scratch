...
Проверено `max|U @ U.T - I| ~ 8e-16`. Ортогональная матрица сохраняет нормы векторов при прохождении линейного слоя (без сжатия и без взрыва), поэтому удерживает сигнал на глубинах, где даже Kaiming начинает плыть — именно поэтому она помогает очень глубоким (50 слоев) ReLU-сетям.
