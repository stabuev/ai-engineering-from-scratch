...
10 эпох. ~97% тестовой точности. Первая эпоха медленная (JIT-компиляция). Эпохи 2-10 быстрые.
...
1. Добавьте dropout в MLP. В JAX dropout требует PRNG-ключ: протащите ключ через прямой проход и разделяйте его для каждого dropout-слоя. Сравните тестовую точность с ним и без него.
