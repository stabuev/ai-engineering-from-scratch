...
- Объяснять, как prompt caching избегает повторной обработки стабильного префикса на каждом запросе.
- Расставлять точки кэширования для максимума попаданий на системном промпте плюс RAG-контексте.
- Считать порог окупаемости и рассуждать о компромиссе TTL кэша.
