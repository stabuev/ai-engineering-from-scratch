...
- Строить mask-aware диффузионную модель и объяснять, почему наивное маскирование неверно.
- Применять SDEdit и InstructPix2Pix для редактирования без дообучения и по инструкции.
- Объяснять, почему редактирование, а не генерация с нуля, — основная часть продакшен-работы с изображениями.
