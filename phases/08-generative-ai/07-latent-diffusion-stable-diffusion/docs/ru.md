...
- Объяснять, почему diffusion в латентном пространстве VAE режет вычисления по сравнению с пиксельным.
- Реализовывать classifier-free guidance с null-токеном и описывать текстовое обусловливание через cross-attention.
- Размещать автоэнкодер (первая стадия) и диффузионную модель (вторая стадия) в пайплайне Stable Diffusion.
