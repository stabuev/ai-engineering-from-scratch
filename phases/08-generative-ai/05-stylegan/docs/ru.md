...
- Объяснять, как отображение z в промежуточное w и его инъекция через AdaIN на каждом разрешении распутывают латентное пространство.
- Реализовывать послойный шум и truncation trick.
- Называть вклад каждого компонента StyleGAN (mapping network, AdaIN, weight demodulation) в фотореализм.
