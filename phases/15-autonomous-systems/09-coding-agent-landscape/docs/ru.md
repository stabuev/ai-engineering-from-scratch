...
- Объяснять, почему результаты SWE-bench выросли в основном за счет обвязки, а не смены базовой модели.
- Сравнивать обвязки агентов (SWE-agent, Cline) при фиксированной модели.
- Критически читать бенчмарк кодинг-агента.
