...
- Обучать небольшой GPT end-to-end на tinyshakespeare с настоящим циклом обучения (bf16 autocast, клиппинг градиента, cosine LR).
- Использовать связанные эмбеддинги и отчитываться по MFU относительно целевых метрик.
- Сэмплировать из обученной модели и критически читать её вывод.
