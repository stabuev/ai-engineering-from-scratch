...
- Объяснять, как Whisper трактует звук как изображение мел-спектрограммы, подаваемое в энкодер-декодер в стиле ViT.
- Прослеживать пайплайн: ресэмплинг → сверточный стем → энкодер → декодер → task-токены → текст.
- Называть, что Whisper делает и чего не делает (например, диаризацию говорящих).
