...
- Реализовывать правила маскирования masked language modeling и прогонять MLM-предсказание на маленьком корпусе.
- Объяснять двунаправленность, токены [CLS]/[SEP] и почему отказались от Next Sentence Prediction.
- Называть задачи 2026, где все еще выбирают энкодер (ModernBERT, cross-encoder).
