# Почему Transformers — проблемы RNN
...
**Тип:** Изучение
**Языки:** Python
**Предварительные требования:** Фаза 3 (ядро deep learning), Фаза 5 · 09 (sequence-to-sequence), Фаза 5 · 10 (механизм attention)
**Время:** ~45 минут

## Цели обучения

- Объяснять, почему трансформеры обрабатывают все токены параллельно, а RNN — последовательно, и что это дало кривым масштабирования после 2017.
- Измерять серийную глубину модели и квадратичную стоимость внимания на длинных последовательностях.
- Называть индуктивные смещения, от которых отказываются трансформеры, и чем они заменены (внимание, контекстное окно).
...
**Ускорение не является константой.** Это разница между `O(N)` последовательной глубиной и `O(1)` последовательной глубиной. На практике transformers обучаются в 5–10× быстрее за эпоху на сопоставимом железе при N=512, и разрыв растет с длиной последовательности, пока вы не упретесь в `O(N²)` стену памяти attention (которую позже исправил Flash Attention — см. Урок 12).
...
### Шаг 1: измерьте последовательную глубину
...
### Шаг 2: посчитайте теоретические операции
...
| Ситуация | Выбор |
|----------|-------|
| Streaming inference, один токен за раз, постоянная память | RNN или state-space model (Mamba, RWKV) |
| Очень длинные последовательности (>1M токенов), где память attention взрывается | Linear attention, Mamba 2, Hyena |
| Edge-устройство без matmul accelerator | Depthwise-separable RNN все еще выигрывает по FLOPs/watt |
| Все остальное (training, batched inference, context до 128K) | Transformer |
...
1. **Легко.** Возьмите `rnn_style` из `code/main.py` и замените скалярное скрытое состояние на вектор скрытых состояний длины 64. Измерьте заново. Насколько последовательный overhead растет с размерностью hidden state?
2. **Средне.** Реализуйте parallel prefix-sum (Hillis-Steele scan) на чистом Python. Проверьте, что он дает тот же численный результат, что и serial scan на длине 1024. Посчитайте глубину.
3. **Сложно.** Перенесите редукцию в стиле attention в PyTorch на GPU. Замерьте оба варианта, перебирая длину последовательности от 64 до 65,536. Постройте график и объясните форму кривой.

## Ключевые термины

| Термин | Как говорят | Что это на самом деле значит |
|------|------------|-------------------------------|
| Recurrence | "RNN последовательны" | Вычисление, где шаг `t` зависит от шага `t-1`, что вынуждает последовательное выполнение вдоль временной оси. |
| Serial depth | "Насколько глубок граф" | Самая длинная цепочка зависимых операций; ограничивает wall-clock даже на бесконечном железе. |
| Attention | "Дать токенам смотреть друг на друга" | Взвешенная сумма `sum_j a_ij v_j`, где `a_ij` получается из score сходства между позициями i и j. |
| Context window | "Сколько видит модель" | Число позиций, которое attention layer может принять на вход; квадратичная стоимость памяти масштабируется здесь. |
| Inductive bias | "Предположения, заложенные в архитектуру" | Априорное предположение о виде данных; CNN предполагают translation invariance, RNN предполагают recency. |
| State-space model | "RNN с алгеброй внутри" | Recurrence, параметризованная для параллельного обучения через структурированные state-space matrices. |
| Quadratic bottleneck | "Почему context так дорог" | Память attention = `O(N²)` по длине последовательности; Flash Attention скрывает константы, но не scaling. |
