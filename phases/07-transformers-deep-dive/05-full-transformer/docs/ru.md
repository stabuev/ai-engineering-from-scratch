...
- Собирать части блока трансформера (внимание, остаточная связь, нормализация, feed-forward) в энкодер- и декодер-блоки.
- Объяснять pre-norm против post-norm и почему современные блоки используют RMSNorm и SwiGLU.
- Собирать небольшой энкодер и декодер и считать их параметры.
