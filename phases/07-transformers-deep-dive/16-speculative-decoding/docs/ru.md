...
- Реализовывать шаг отклонения, остаточное распределение и bonus-токен в speculative decoding.
- Объяснять, что коэффициент принятия α определяет в сквозном ускорении.
- Противопоставлять стратегии черновика Medusa (без модели-черновика) и EAGLE (переиспользование скрытых состояний).
