...
- Прослеживать стек TTS 2026: текст → токены → mel → форма волны.
- Фонемизировать вход, запускать TTS-модель по умолчанию на CPU и собирать вокодер HiFi-GAN с нуля.
- Оценивать качество TTS через MOS и SECS.
