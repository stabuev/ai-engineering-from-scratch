...
> Все от «лай собаки против сирены» до «какой это язык» — аудиоклассификация. Признаки — mels. Архитектура меняется каждое десятилетие. Оценка остается AUC, F1 и полнотой по классам.
...
- Извлекать признаки log-mel и классифицировать звук — от baseline k-NN до CNN.
- Бороться с дисбалансом классов с помощью Mixup и SpecAugment.
- Оценивать правильными метриками (AUC, F1, mAP).
...
Главная сложность — не сеть. Это данные. В аудиодатасетах жесткий дисбаланс классов, сильный сдвиг домена (чистое против шумного) и шумные метки (кто решил, что это «городской гул», а не «шум ресторана»?). 80% задачи — кураторство, аугментация и оценка, а не замена CNN на Transformer.

## Концепция

![Audio classification ladder: k-NN on MFCCs to AST to BEATs](../assets/audio-classification.svg)

**k-NN на MFCC (бейзлайн 1990-х).** Сплющите MFCC по клипу, посчитайте cosine similarity к размеченному банку и верните голосование большинства по top K. Неожиданно силен на чистых маленьких датасетах (Speech Commands, ESC-50). Работает без GPU.
...
**Audio Spectrogram Transformer, AST (2021-2024).** Разбейте log-mel на патчи (например, 16×16), добавьте позиционные эмбеддинги, подайте в ViT. State of the art на AudioSet (mAP 0.485) для обучения с учителем.

**BEATs и WavLM-base (2024-2026).** Self-supervised pretraining на миллионах часов. Дообучение под вашу задачу требует 1-10% тех размеченных данных, которые понадобились бы раньше. В 2026 году это дефолтная стартовая точка для неречевого аудио. BEATs-iter3 обходит AST на 1-2 mAP на AudioSet при четверти вычислений.
...
ESC-50: 50 классов, по 40 клипов — сбалансированно, легко. UrbanSound8K: 10 классов, дисбаланс 10:1. AudioSet: 632 класса с длинным хвостом 100,000:1. Рабочие техники:
...
- Взаимоисключающая многоклассовая классификация (Speech Commands): top-1 accuracy, top-5 accuracy.
- Многоклассовая многометочная классификация (AudioSet, UrbanSound-style): mean average precision (mAP).
- Сильный дисбаланс: полнота по классам + macro F1.
...
2. **Средне.** Замените `summarize` на [mean, var, skew, kurtosis]. Побеждает ли pooling по 4 моментам mean+var на том же синтетическом датасете?
