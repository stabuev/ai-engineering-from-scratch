...
- Строить real-time аудио-пайплайн с кольцевым буфером, VAD-гейтом, потоковым ASR и обработкой прерываний.
- Объяснять, почему real-time — это обработать следующие 20 мс до прихода очередных 20.
- Называть подводные камни латентности (jitter buffer, подавление эха, barge-in, full duplex).
