...
Эффективная версия быстрее: ускоритель запускает один matmul вместо трех. Ее также легче инициализировать, потому что три подматрицы живут в одном тензоре-параметре и инициализируются вместе.
...
Измерение d_head остается последним, чтобы матмул скоров `Q @ K.transpose(-2, -1)` его свернул. Результат — пер-головные скоры внимания `(B, H, T, T)`.
...
Он не добавляет feed-forward-блок. Слой трансформера в настоящей модели — это внимание, за которым идет двухслойный MLP с residual-связью и layer norm вокруг каждого. Их добавляет следующий урок.
