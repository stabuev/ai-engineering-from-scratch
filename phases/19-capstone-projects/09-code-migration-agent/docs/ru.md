...
- Построить агента миграции уровня репозитория (например, апгрейд языка или версии).
- Сочетать детерминированные AST-переписывания с LLM-починкой и верификацией через тесты.
- Проверять каждый перенесённый модуль на его наборе тестов.
