...
> Каждая серьезная engineering org в 2026 году запускает internal code search, который понимает смысл, а не только строки. Sourcegraph Amp, Cursor's codebase answers, Augment's enterprise graph, Aider's repomap, Pinterest's internal MCP — форма одна и та же. Ingest many repos, parse with tree-sitter, embed function- and class-level chunks, hybrid-search, re-rank, answer with citations. Этот capstone предлагает собрать систему, которая обрабатывает 2M lines of code across 10 repos и выдерживает incremental re-indexing на каждый git push.

**Тип:** Capstone
**Языки:** Python (ingestion), TypeScript (API + UI)
**Предварительные требования:** Phase 5 (NLP foundations), Phase 7 (transformers), Phase 11 (LLM engineering), Phase 13 (tools), Phase 17 (infrastructure)
...
Вы изучите это, индексируя реальный fleet — не один tutorial repo — и измеряя MRR@10, citation faithfulness и incremental freshness. Failure modes инфраструктурные: 100k-file monorepo, push, который retouches half the files, query, которому нужно пересечь four repos, чтобы ответить корректно.

## Концепция

AST-aware ingestion pipeline parses each file with tree-sitter, extracts function and class nodes, and chunks at node boundaries rather than fixed token windows. Каждый chunk получает три representations: dense embedding (Voyage-code-3 or nomic-embed-code), sparse BM25 terms и short natural-language summary. Summary добавляет третью retrievable modality — users ask "how is X authorized", а summary mentions "authz", даже если code contains only `check_permission`.

Retrieval is hybrid. Query запускает dense и BM25 searches, merges top-k и передает union в cross-encoder re-ranker (Cohere rerank-3 or bge-reranker-v2-gemma-2b). Re-ranked list идет в long-context synthesizer (Claude Sonnet 4.7 with prompt caching, or Llama 3.3 70B self-hosted) с instructions to cite every claim by file and line range. Answers without citations rejected by a post-filter.

Incremental freshness — это infrastructure problem. Git push triggers a diff: какие files changed, какие symbols changed. Only affected chunks re-embed. Affected cross-file symbol edges (imports, method calls) recomputed. Index stays consistent without reprocessing 2M lines each commit.
...
5. **Symbol graph.** Для каждого chunk запишите edges: imports (this file uses symbol Y from repo Z), calls (this function calls method M on class C), inheritance. Store in kuzu. Used at query time to expand retrieval across repo boundaries.

6. **Query agent.** LangGraph with three nodes. `retrieve` запускает dense + BM25 in parallel, deduplicates by (repo, path, symbol). `rerank` runs the cross-encoder on top-50 and keeps top-10. `synth` calls Claude Sonnet 4.7 with the reranked chunks in context, caches the system prompt, requires file:line citations.
...
Deliverable skill `outputs/skill-codebase-rag.md`. По corpus of repos он поднимает ingestion pipeline, hybrid index и query agent, и возвращает cited answer для любого cross-repo question. Rubric:
...
1. Замените Voyage-code-3 на nomic-embed-code self-hosted. Измерьте MRR@10 delta. Сообщите, закрывается ли gap with re-ranking enabled.
...
|------|-----------------|------------------------|
| AST-aware chunking | "Function-level splits" | Разбиение code по tree-sitter node boundaries вместо fixed token windows |
...
| Citation faithfulness | "Grounded answer rate" | Доля claims, которые user can verify by clicking the anchor and reading the referenced span |
