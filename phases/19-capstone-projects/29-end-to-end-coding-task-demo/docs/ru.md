...
> Кульминация трека A. Этот урок сшивает цепочку gates, sandbox, eval-харнес и OTel-span'ы в одного работающего coding-агента, чинящего настоящий (маленький, fixture-масштаба) баг в многофайловом Python-проекте. Агент — детерминированная политика, а не LLM; подстановка делает урок воспроизводимым и показывает, что самым интересным все это время был харнес. Контракт идентичен: настоящая модель подключается в шов политики.
...
- Обеспечить глобальный бюджет шагов плюс токеновый бюджет наблюдений на всем end-to-end-прогоне.
- Излучить полные OTel GenAI-трейсы и Prometheus-метрики за весь прогон.
- Убедиться, что агент решает фикстуру менее чем за 12 шагов с нулем срабатываний gates на легальных инструментах.
...
Каждое состояние соответствует одному tool call. Каждый tool call проходит через цепочку gates. Если вызов отклонен, агент фиксирует отказ в трейсе и останавливается.

Баг фикстуры — off-by-one в `fizz.py`. Детерминированная политика распознает баг по сообщению о провале теста через regex и излучает исправленный файл. Замена политики на LLM контракт харнеса не меняет.
...
Настоящая LLM требует API-ключа, сетевого вызова и неверифицируемой стохастичности. Урок заботится о харнесе. Подстановка детерминированной политики позволяет уроку работать на любом ноутбуке разработчика с нулем внешних зависимостей и дает тестовому набору утверждать точные счетчики шагов.
...
Ноль отказов gates на легальных инструментах. (Агент ни разу не выдумал запрещенное имя инструмента.)
...
Этот урок — интеграция. Урок 25 написал цепочку gates. Урок 26 — sandbox. Урок 27 — eval-харнес. Урок 28 — observability. Урок 29 доказывает, что они работают как система. Настоящий агентский харнес растет отсюда: замените детерминированную политику моделью, комплектную фикстуру — задачей на настоящем репозитории, JSONL-экспортер — OTLP.
...
Демо печатает пошаговый трейс, финальный eval-отчет и Prometheus-экспозицию. Код выхода — ноль. Тесты покрывают переходы состояний политики, отказы gates на синтетических tool calls, end-to-end-прогон на комплектной фикстуре и инварианты бюджета шагов.
