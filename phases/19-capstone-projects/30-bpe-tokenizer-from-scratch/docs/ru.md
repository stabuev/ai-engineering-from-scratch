...
- Реализовать детерминированную таблицу слияний и применить ее к свежему тексту, получив поток subword-id.
...
Кодировщик принимает флаг, разрешающий специальные токены во входе. С выключенным флагом строки `<|endoftext|>` и `<|pad|>` токенизируются как байты, которыми они записаны. С включенным — буквальные строки маппятся на зарезервированные id и не подлежат никакому слиянию.
