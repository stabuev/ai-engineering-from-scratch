...
> Правило — это имя, предикат и объяснение. Все, чему не хватает одного из трех, — вайб, а не правило.
...
- Представить конституцию декларативным YAML-файлом именованных правил с предикатами, severity и объяснениями.
- Компоновать атомарные предикаты через all_of, any_of и not_.
- Гейтить правила через applies_when, чтобы неприменимое правило записывалось как not_applicable, а не violation.
- Применять детерминированный фиксер и излучать структурный diff черновик-в-ревизию.
...
`python3 main.py`. Демо гоняет три черновика через движок, печатает нарушения, гоняет фиксер, печатает diff и пишет `outputs/rules_report.json`. У одной фикстуры неприменимое правило (нет код-блока в черновике), и отчет показывает `not_applicable` для этого правила, чтобы команда видела: движок вычислил его явно.
