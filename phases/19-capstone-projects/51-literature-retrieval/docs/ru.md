...
> Гипотеза дешева. Дорогое — знать, не доказал ли ее уже кто-то. Постройте слой retrieval, отвечающий на этот вопрос до того, как раннер поднимет sandbox.
...
Retrieval-клиент владеет обоими проходами и слиянием. Вызывающий отдает ему запрос и получает ранжированный список, где каждая запись несет пер-статейные поля скоров (`bm25_score`, `graph_distance`, `recency_score`, `final_score`), объясняющие ранжирование.
...
Граф строится один раз из корпуса. Прямые ребра идут от статьи к ее ссылкам. Обратные — от статьи к ее цитированиям. Обход — поиск в ширину, засеянный топ-попаданиями BM25, с потолком в два прыжка.
