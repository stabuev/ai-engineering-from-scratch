...
- Оценивать функцию ценности только из сэмплированных эпизодов методом first-visit Monte Carlo — без модели среды.
- Реализовывать ε-жадное on-policy MC-управление и сравнивать его с эталоном динамического программирования.
- Различать on-policy и off-policy обучение и объяснять, где появляется importance sampling.
