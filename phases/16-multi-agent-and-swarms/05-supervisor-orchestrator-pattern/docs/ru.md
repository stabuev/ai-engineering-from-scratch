...
- Строить паттерн supervisor / orchestrator-worker: ведущий планирует и делегирует, воркеры работают параллельно.
- Объяснять, почему он выигрывает, и инженерные уроки из системы Anthropic Research.
- Диагностировать его режимы отказа.
