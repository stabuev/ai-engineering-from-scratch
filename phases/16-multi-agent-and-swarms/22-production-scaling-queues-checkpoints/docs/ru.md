...
- Масштабировать мультиагентную систему через durable-исполнение, очереди и чекпоинты.
- Сравнивать async против thread-per-job и очереди по агенту (MegaAgent).
- Писать чекпоинт после каждого супершага (паттерн рантайма LangGraph).
