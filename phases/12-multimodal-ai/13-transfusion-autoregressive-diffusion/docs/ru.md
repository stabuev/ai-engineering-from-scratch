...
Спор о дискретных и непрерывных токенах изображения старше LLM. Непрерывные представления (raw pixels, VAE latents) сохраняют детали. Дискретные токены (VQ indices) подходят родному словарю трансформера, но теряют детали на этапе квантизации.
