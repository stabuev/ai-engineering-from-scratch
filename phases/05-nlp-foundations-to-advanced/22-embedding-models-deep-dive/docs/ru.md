...
- Генерировать плотные, разреженные и multi-vector эмбеддинги (Sentence-BERT, BGE-M3).
- Применять Matryoshka-усечение, меняя размерность на стоимость.
- Критически читать лидерборд MTEB.
