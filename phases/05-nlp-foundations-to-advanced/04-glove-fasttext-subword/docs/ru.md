...
- Факторизовать матрицу совстречаемости (GloVe), эмбеддить подслова (FastText) и учить BPE-словарь.
- Объяснять, как подсловные эмбеддинги решают проблему OOV.
- Выбирать между GloVe, FastText и BPE под задачу.
