...
**Тип:** Сборка
**Языки:** Python
**Предварительные требования:** Фаза 5 · 01 (Text Processing), Фаза 2 · 14 (Naive Bayes)
**Время:** ~45 минут

## Цели обучения

- Строить n-граммную языковую модель со сглаживанием Лапласа и Кнезера-Нея.
- Генерировать текст сэмплированием и измерять перплексию.
- Объяснять backoff и вероятность продолжения.
...
![N-граммная модель: считать, сглаживать, генерировать](../assets/ngram.svg)
...
1. **Легко.** Обучите триграммную LM на корпусе Shakespeare из 1,000 предложений. Сгенерируйте 20 предложений. Локально они будут правдоподобными, но глобально несвязными. Это каноническая демонстрация.
2. **Средне.** Реализуйте perplexity для вашей KN-модели на отложенном разбиении Shakespeare. Сравните с Laplace. Вы должны увидеть, что KN снижает perplexity на 30-50%.
3. **Сложно.** Постройте триграммный корректор орфографии: по слову с ошибкой и его контексту генерируйте исправления и ранжируйте их по контекстной вероятности LM. Оцените на корпусе орфографических ошибок Birkbeck (публичный).
