...
- Строить TextCNN и LSTM-классификатор в PyTorch.
- Объяснять признаки n-грамм (свёртки) против памяти (рекуррентность) и проблему затухающего градиента.
- Объяснять, почему внимание вытеснило оба — и где они всё ещё подходят для ограниченного железа.
