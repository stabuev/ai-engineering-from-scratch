...
Трекер статусов для каждой фазы и каждого урока. Таблицы в этом файле генерируются
из `lessons.json` скриптом `site/build.js` — правьте манифест и запускайте
`node site/build.js`, а не таблицы руками.

Общее оценочное время: ~486 часов уроков (фазы 0–18) + ~627 часов capstone-проектов, в своем темпе.

**Легенда:** ✅ Завершено &nbsp;·&nbsp; 🚧 В работе &nbsp;·&nbsp; ⬚ Запланировано &nbsp;·&nbsp; Квиз: ✓ есть / — пока нет
...
## Фаза 19: Capstone-проекты — ✅ (~627 часов)
...
**Итого: 20 фаз, 502 уроков | 502 завершено | ~1113 часов по оценке (включая capstone-проекты)**
